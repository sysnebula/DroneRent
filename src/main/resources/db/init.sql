...
CREATE TABLE drone (
    ...
    flight_hours DECIMAL(10, 2) DEFAULT 0 COMMENT '累计飞行时长（小时）',
...
CREATE TABLE customer (
    ...
    credit_score INT DEFAULT 100 COMMENT '信用分数（满分100）',
    blacklist TINYINT NOT NULL DEFAULT 0 COMMENT '黑名单标识（0-否，1-是）',
...
CREATE TABLE orders (
    ...
    damage_fee DECIMAL(10, 2) DEFAULT 0 COMMENT '损坏赔偿费',
    overdue_fee DECIMAL(10, 2) DEFAULT 0 COMMENT '逾期费用',
    discount_amount DECIMAL(10, 2) DEFAULT 0 COMMENT '优惠金额',
    status VARCHAR(20) NOT NULL DEFAULT 'PENDING' COMMENT '订单状态（PENDING-待支付，PAID-已支付，RENTING-租赁中，COMPLETED-已完成，CANCELLED-已取消，REFUNDED-已退款）',
...
CREATE TABLE maintenance_record (
    ...
    parts_replaced TEXT COMMENT '更换零件（JSON数组）',
...
CREATE TABLE inventory_log (
    ...
    drone_id BIGINT NOT NULL COMMENT '无人机ID',
    change_type VARCHAR(20) NOT NULL COMMENT '变动类型（PURCHASE-采购，RENT_OUT-出租，RETURN-归还，MAINTENANCE-维修，SCRAP-报废）',
...
-- 插入测试无人机数据
INSERT INTO drone (drone_no, brand, model, serial_number, purchase_date, purchase_price, daily_rental_price, deposit_amount, status, flight_hours, total_flights, description) 
VALUES 
('DRONE001', 'DJI', 'Mavic 3', 'SN2024001', '2024-01-15', 12999.00, 299.00, 3000.00, 'IDLE', 50.5, 120, '大疆 Mavic 3 专业航拍无人机'),
('DRONE002', 'DJI', 'Air 2S', 'SN2024002', '2024-02-20', 7999.00, 199.00, 2000.00, 'IDLE', 30.2, 80, '大疆 Air 2S 轻便航拍无人机'),
('DRONE003', 'Autel', 'EVO II Pro', 'SN2024003', '2024-03-10', 15999.00, 399.00, 4000.00, 'IDLE', 20.0, 50, '道通 EVO II Pro 专业级无人机');
